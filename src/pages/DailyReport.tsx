  ...
  if (!identifier) return (
    <div className="p-8 text-center text-rose-400 font-bold bg-[#0F0E19] min-h-screen flex flex-col items-center justify-center space-y-4">
      <div className="max-w-2xl whitespace-pre-wrap text-left font-mono text-xs opacity-70 border border-rose-500/30 p-4 rounded bg-rose-500/5">
        ...
        {"\n"}
        Evidence: The three edge functions disagree on how they resolve the `director_slug`. `daily-team-info/index.ts` and `director-weekly/index.ts` both use a new `directorSlugMatches(name, requested)` helper that accepts either the full-name slug OR the first-name slug of the director. However, `submit-daily-report/index.ts` (line 69) still uses the strict check `slugify(b.name || "") === body.director_slug`, matching only the full-name slug. In `src/pages/PublicDirectorCheckpoint.tsx` the "Preencher meu daily" links pass `?director=${encodeURIComponent(slug)}` where `slug` comes straight from the URL `useParams`, and in `src/pages/DailyReport.tsx` that `directorParam` is forwarded verbatim to both `daily-team-info` (used to unlock the form via `director_ok`) and `submit-daily-report`. So any URL slug that resolves the director by first-name (e.g. `/diretor/lucas` when the director is "Lucas Silva") passes the initial load and roster fetch but is rejected at submit time, returning {"{ error: \"Não autorizado (PIN inválido ou link do diretor inválido).\" }"} and discarding the filled entries.
      </div>
